...
local function Step_OnClick(self, button)
	...
	if not ZGV.CurrentGuide.steps[self.stepnum] or not ZGV.CurrentGuide.steps[self.stepnum]:AreRequirementsMet() then
		return
	end
...
function ZygorGuidesViewerFrame_OnLoad(self)
	...
	local tracker =
		ZygorGuidesViewerFrameScrollScrollBar:CreateTexture("ZygorGuidesViewerFrameScrollScrollBarTrackerTexture")
...
function ZygorGuidesViewerFrame_OnUpdate(self, elapsed)
	...
	if
		...
		or Border:IsShown() and (MouseIsOver(TitleBar, 0, 0, 50, -50) or not ZGV.CurrentGuide) and not ZGV.loading
	then
		if locked then
			xPos, yPos = GetCursorPosition()
			if (not but.oldxPos or not but.oldyPos or xPos ~= but.oldxPos or yPos ~= but.oldyPos) and but.delay > 0 then
				but.delay = 0
			end
